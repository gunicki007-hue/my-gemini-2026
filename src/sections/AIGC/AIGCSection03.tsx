    ...
    <section className="relative w-full bg-[#050505] z-30">
      <div className="w-full grid grid-cols-1 lg:grid-cols-10 items-start px-6 md:px-[60px] lg:px-[80px] gap-12 lg:gap-20">
        ...
        <div className="col-span-1 lg:col-span-6 w-full flex flex-col py-12 lg:py-[15vh] gap-12 lg:gap-[calc(30vh-80px)]">
          ...
          >
            <div className="w-full flex flex-col p-6 md:p-8 lg:p-10 text-sm">
              {/* Header */}
              <div className="text-center mb-8 md:mb-10">
                <h3 className="text-white text-lg md:text-2xl font-bold tracking-widest">{isEn ? '5-Step SOP Workflow' : '5步跑通SOP 工作流'}</h3>
              </div>
              
              {/* Table Header */}
              <div className="hidden md:grid grid-cols-4 gap-4 mb-4 text-white/50 text-xs font-medium tracking-wide">
                <div></div>
                ...
                {[
                  ...
                  <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-4 items-center border-b border-white/5 pb-6 md:pb-4 last:border-0 last:pb-0">
                    <div className="flex flex-row md:flex-col gap-4 md:gap-1 items-baseline md:items-start">
                      <div className="text-3xl md:text-4xl font-black text-white/20 italic font-sans">{row.step}</div>
                      <div className="text-white font-medium text-[13px] md:text-sm">{isEn ? row.enTitle : row.title}</div>
                    </div>
            ...
            <div className="w-full bg-[#111111] rounded-xl overflow-hidden border border-white/5 flex flex-col shadow-2xl">
              <div className="w-full overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[700px]">
                  <thead>
                    <tr className="border-b border-white/10 text-white/40 text-[12px] md:text-[13px] tracking-widest font-medium">
                      ...
                      <th className="py-4 md:py-5 px-4 font-sans text-center">
                        <div className="flex flex-col items-center">
                          ...
                          <span className="text-[10px] mt-0.5">(1-5)</span>
                        </div>
                      </th>
                      <th className="py-4 md:py-5 px-4 font-sans text-center">
                        <div className="flex flex-col items-center">
                        ...
                        <td className="py-5 md:py-6 px-6">
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex flex-col gap-1.5">
                              <span className="text-[14px] md:text-[15px] font-bold tracking-wide">{row.title}</span>
                              <span className="text-[11px] md:text-[12px] text-white/50 leading-snug font-light max-w-[280px]">{row.desc}</span>
                            </div>
                          </div>
                        </td>
                        <td className="py-5 md:py-6 px-4 text-center text-[14px] md:text-[15px] font-medium text-white/90">{row.i}</td>
                        <td className="py-5 md:py-6 px-4 text-center text-[14px] md:text-[15px] font-medium text-white/90">{row.s}</td>
                        <td className="py-5 md:py-6 px-4 text-center text-[15px] md:text-[17px] font-bold text-[#E5B979]">{row.score}</td>
                        <td className="py-5 md:py-6 px-6 text-center">
                          <div className={`flex items-center justify-center gap-1.5 font-bold ${row.pColor}`}>
                            {row.medal && <span className="text-[16px]">{row.medal}</span>}
                            <span>{row.p}</span>
                          </div>
                        </td>
                      </tr>
              ...
              <div className="w-full bg-[#161616] py-3.5 px-6 border-t border-white/5 flex items-center gap-2 text-[11px] md:text-[12px] text-white/40 font-light">
        ...
        >
          <div className="flex flex-col items-center gap-6">
            ...
            <div className="text-center mb-[30px]">
              <h2 className="text-white text-2xl md:text-3xl lg:text-4xl font-bold tracking-widest font-sans">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="w-full bg-white/[0.02] backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-[32px] p-6 lg:p-8 relative overflow-x-auto h-full flex flex-col justify-center">
                ...
                <table className="w-full h-full text-left border-collapse min-w-[500px] relative z-10">
                  <thead>
                    <tr className="border-b border-white/10 text-white/40 text-[12px] md:text-[13px] tracking-widest uppercase font-medium">
                  ...
                  <tbody className="divide-y divide-white/5 text-white/90">
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Tedious ops' : '操作繁琐'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Manual input & path selection' : '手动输入金额、选路径'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'AI remembers amounts, generates shortcuts, reduces steps by 50%' : 'AI记忆常用金额，生成快捷选项，步骤减少50%'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'High decision cost' : '决策成本高'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Manual judgment of fee/network' : '手动判断手续费/网络'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'AI proactively suggests optimal timing' : 'AI主动提示最优操作时机'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Weak risk perception' : '风险感知弱'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Normal text warning' : '普通文字提醒'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'Full-screen strong warning + 2FA for large/abnormal tx' : '大额/异常交易触发全屏强提醒+二次验证'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Hard to understand info' : '信息理解难'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Pure data bills' : '纯数据账单'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'AI translates to plain interpretation, reducing cost' : 'AI转化为通俗解读，降低理解成本'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="w-full bg-white/[0.02] backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-[32px] p-6 lg:p-8 relative overflow-x-auto h-full flex flex-col justify-center">
                ...
                <table className="w-full h-full text-left border-collapse min-w-[500px] relative z-10">
                  ...
                  <tbody className="divide-y divide-white/5 text-white/90">
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Product Arch' : '产品架构'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Rigid structure, fragmented modules, traditional stacking' : '结构固化，模块割裂，整体偏向传统功能堆叠'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'AI as native foundation, integrated full-link connection' : 'AI作为底层原生架构，整体链路一体化联动'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Iteration Mode' : '迭代模式'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Long cycles, fully manual requirement sorting' : '迭代周期长，需求梳理全靠人工，优化滞后'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'Flexible & efficient, fast iteration via AI assistance' : '迭代灵活高效，依托智能辅助快速梳理需求、迭代方案'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Risk Handling' : '风险处理'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Passive control, post-event review, lacks preview' : '被动式风控，事后问题复盘，缺少前置预判'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'Proactive smart risk control, early identification' : '全链路前置智能风控，提前识别规避业务隐患'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Expansion Cap.' : '拓展能力'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Weak adaptation, limited personalization' : '场景适配弱，个性化调整空间小，扩展性有限'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'High compatibility, on-demand personalization' : '场景兼容度高，可按需做个性化适配，延展性强'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-2 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Design Logic' : '设计逻辑'}</td>
                      <td className="py-6 md:py-8 px-4 text-[14px] md:text-[15px] font-light text-white/60 leading-relaxed">{isEn ? 'Business-centric, lacks global experience control' : '以业务功能为核心，缺少全局体验统筹'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-2 text-[14px] md:text-[15px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'Dual dimension: full-link experience + business value' : '以用户全链路体验+业务价值双重维度做统筹'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-10 items-center gap-12 lg:gap-20">
          ...
          <div className="col-span-1 lg:col-span-4 flex flex-col z-10 w-full shrink-0">
            ...
            <motion.h2
              ...
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
              className={`w-full mb-6 lg:mb-8 text-white text-[30px] md:text-[38px] xl:text-[46px] font-extrabold tracking-tighter leading-[1.1] ${isEn ? 'font-sans' : 'font-puhuiti'}`}
            >
            ...
            <motion.div
              ...
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 1, ease: [0.16, 1, 0.3, 1] }}
              className={`space-y-6 text-white/70 ${isEn ? 'font-sans font-light tracking-[0.015em] leading-[1.6] text-[16px]' : 'font-sans font-light tracking-[0.03em] leading-[1.8] text-[15px] xl:text-[16px] text-justify'}`}
            >
              <p>
                {isEn ? (
            ...
            >
              ...
              <div className="w-full bg-white/[0.02] backdrop-blur-3xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-[32px] p-6 lg:p-8 xl:p-10 relative overflow-x-auto flex flex-col justify-center">
                ...
                <table className="w-full text-left border-collapse min-w-[600px] relative z-10">
                  <thead>
                    <tr className="border-b border-white/10 text-white/40 text-[12px] md:text-[14px] tracking-widest uppercase font-medium">
                  ...
                  <tbody className="divide-y divide-white/5 text-white/90">
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Basic Work' : '基础工作'}</td>
                      <td className="py-6 md:py-8 px-4 text-[13px] md:text-[14px] font-light text-white/60 leading-relaxed">{isEn ? 'All done manually, repetitive tasks take long' : '全部人工手动完成，重复事务耗时长'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[13px] md:text-[14px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'AI takes over repetitive writing, screening, sorting work' : 'AI承接基础编写、筛查、整理类重复工作'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Imp. Efficiency' : '落地效率'}</td>
                      <td className="py-6 md:py-8 px-4 text-[13px] md:text-[14px] font-light text-white/60 leading-relaxed">{isEn ? 'Tedious process, high trial error cost, long cycle' : '流程繁琐，试错成本高，整体周期冗长'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[13px] md:text-[14px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'Significantly reduces time in basic links, overall efficiency improved' : '大幅压缩基础环节耗时，整体效率显著提升'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Adjustment' : '调整优化'}</td>
                      <td className="py-6 md:py-8 px-4 text-[13px] md:text-[14px] font-light text-white/60 leading-relaxed">{isEn ? 'High modification cost, iteration takes a lot of effort' : '修改成本高，迭代调整耗费大量精力'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[13px] md:text-[14px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'Quickly generate drafts, easier modification and iteration' : '快速生成初稿，修改迭代更轻便'}</td>
                    </tr>
                    <tr className="hover:bg-white/[0.03] transition-colors duration-500 group">
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[14px] md:text-[15px] font-bold tracking-wide group-hover:text-white transition-colors">{isEn ? 'Core Driven' : '核心主导'}</td>
                      <td className="py-6 md:py-8 px-4 text-[13px] md:text-[14px] font-light text-white/60 leading-relaxed">{isEn ? 'Full process manned by humans, energy distracted by trivial work' : '全流程人力包揽，精力被琐碎工作分散'}</td>
                      <td className="py-6 md:py-8 pl-4 pr-4 text-[13px] md:text-[14px] font-normal tracking-wide text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{isEn ? 'AI handles execution, humans focus on direction, decision, quality control' : 'AI负责执行落地，人专注方向、决策、质量把控'}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
