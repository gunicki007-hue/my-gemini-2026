          ...
          >
            ...
            <div className="w-full flex-1 flex flex-col relative z-10 justify-between">
              ...
              <div className="w-full pb-6 md:pb-8 flex items-center border-b border-white/[0.05]">
                <div className="flex-[0.6] text-white/70 text-[16px] md:text-[16px] tracking-tighter">{isEn ? 'DIMENSION' : '维度'}</div>
                <div className="flex-1 text-white/70 text-[16px] md:text-[16px] tracking-tighter pl-4">{isEn ? 'TRADITIONAL MODE' : '传统方式'}</div>
                <div className="flex-1 text-white/70 text-[16px] md:text-[16px] tracking-tighter pl-4">{isEn ? 'HYBRID ORCHESTRATION' : '混合智能体编排'}</div>
              </div>

              <div className="w-full py-6 md:py-8 flex items-center border-b border-white/[0.05]">
                <div className="flex-[0.6] text-white/70 text-[9px] md:text-[10px] font-light">{isEn ? 'Concept Trial Cycle' : '概念试错周期'}</div>
                <div className="flex-1 text-white/80 text-[16px] md:text-[16px] pl-4 font-light tracking-wide">{isEn ? '~2 weeks (Manual)' : '约 2 周（人工推演与绘图）'}</div>
                <div className="flex-1 text-[#D5B667] text-[16px] md:text-[16px] pl-4 font-light tracking-wide opacity-90">{isEn ? 'Overnight (Autonomous computing)' : '隔夜完成（算力自主演算）'}</div>
              </div>

              <div className="w-full py-6 md:py-8 flex items-center border-b border-white/[0.05]">
                <div className="flex-[0.6] text-white/70 text-[9px] md:text-[10px] font-light">{isEn ? 'Brand Guideline Exec.' : '品牌规范执行'}</div>
                <div className="flex-1 text-white/80 text-[16px] md:text-[16px] pl-4 font-light tracking-wide">{isEn ? 'Manual check, error-prone' : '依赖人工检查，易产生视觉偏差'}</div>
                <div className="flex-1 text-[#D5B667] text-[16px] md:text-[16px] pl-4 font-light tracking-wide opacity-90">{isEn ? 'Algorithm locked, 0 deviation' : '模型算法锁死，实现 0 偏差复用'}</div>
              </div>

              <div className="w-full py-6 md:py-8 flex items-center border-b border-white/[0.05]">
                <div className="flex-[0.6] text-white/70 text-[9px] md:text-[10px] font-light">{isEn ? 'Team Capacity Limit' : '团队产能上限'}</div>
                <div className="flex-1 text-white/80 text-[16px] md:text-[16px] pl-4 font-light tracking-wide">{isEn ? 'Limited by team size' : '受限于团队规模与加班时长'}</div>
                <div className="flex-1 text-[#D5B667] text-[16px] md:text-[16px] pl-4 font-light tracking-wide opacity-90">{isEn ? 'Limited by compute (Near infinite)' : '受限于算力配置（近乎无限）'}</div>
              </div>

              <div className="w-full pt-6 md:pt-8 flex items-center">
                <div className="flex-[0.6] text-white/70 text-[9px] md:text-[10px] font-light">{isEn ? 'Marginal Cost' : '单产边际成本'}</div>
                <div className="flex-1 text-white/80 text-[16px] md:text-[16px] pl-4 font-light tracking-wide">{isEn ? 'Linear surge with business' : '随业务需求量呈线性暴增'}</div>
                <div className="flex-1 text-[#D5B667] text-[16px] md:text-[16px] pl-4 font-light tracking-wide opacity-90">{isEn ? 'Zero Marginal Cost' : '趋近于零（Zero Marginal Cost）'}</div>
              </div>
            </div>
