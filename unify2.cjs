...
const targetEn = "text-[50px] sm:text-[70px] md:text-[90px] xl:text-[100px] font-black font-sans tracking-tight leading-[1.1]";
const targetZh = "text-[40px] sm:text-[60px] md:text-[80px] xl:text-[90px] font-black font-puhuiti tracking-tighter leading-[1.1]";
...
function walk(dir) {
  for (const item of fs.readdirSync(dir)) {
    ...
    if (fs.statSync(p).isDirectory()) {
      ...
      // Also clean up any other missed titles that we want to hit
      content = content.replace(/`text-center uppercase w-full \S*?\s*\$\{isEn \? 'text.*?' : 'text.*?'\}`/g, 
        "`text-center uppercase w-full ${isEn ? '" + targetEn + "' : '" + targetZh + "'}`"
      );
